...
import PomodoroStatsTable from './PomodoroStatsTable';
...
export default function SummaryTables({ tasks, categories, pomodoroHistory, workouts }: Props) {
  const [activeTable, setActiveTable] = useState<'tasks' | 'categories' | 'pomodoro' | 'difficulty' | 'workouts'>('tasks');
  const [selectedTaskSessions, setSelectedTaskSessions] = useState<{ title: string; sessions: PomodoroSession[] } | null>(null);
  ...
  return (
    <section>
      ...
      {activeTable === 'categories' && <CategoryDetailTable tasks={tasks} categories={categories} />}
      {activeTable === 'pomodoro' && (
        <PomodoroStatsTable
          pomodoroHistory={pomodoroHistory}
          onSelectTaskSessions={(title, sessions) => setSelectedTaskSessions({ title, sessions })}
        />
      )}
      {activeTable === 'difficulty' && <DifficultyTable tasks={tasks} />}
      {activeTable === 'workouts' && <WorkoutSummaryTable workouts={workouts} />}

      {/* Reusable Pomodoro Sessions Detail Modal */}
      {selectedTaskSessions && (
        <PomodoroHistoryModal
          title={selectedTaskSessions.title}
          sessions={selectedTaskSessions.sessions}
          onClose={() => setSelectedTaskSessions(null)}
        />
      )}
    </section>
  );
